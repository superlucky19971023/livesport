...
const AmericanFootballPage: React.FC = () => {
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      try {
        const apiResponse = await fetchData(`american-football/matches/${now.date}/${now.month}/${now.year}`);
        const temp: any[] = [];
        apiResponse.events.forEach((element: any) => {
          if (element.awayTeam.country.name == element.homeTeam.country.name && element.awayTeam.sport.name == "American football" && element.homeTeam.sport.name == "American football") {
            temp.push(element);
          }
        });
        if (temp.length != 0) {
      ...
      <div>
        {
          countryArr.map((value: any, index: number) => {
            return <CountryCard label={value.countryName} flag={value.countryFlag} count={value.matchCount} competitions_name={value.competitions_name} competitions={value.competitions} key={index + "American_Football_PlayCard"} />
          })
        }
      </div>
    </div>
