...
const CricketPage: React.FC = () => {
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      try {
        const apiResponse = await fetchData(`cricket/matches/${now.date}/${now.month}/${now.year}`);
        const temp: any[] = [];
        apiResponse.events.forEach((element: any) => {
          if (element.awayTeam.country.name == element.homeTeam.country.name && element.awayTeam.sport.name == "Cricket" && element.homeTeam.sport.name == "Cricket") {
            temp.push(element);
          }
        });
        if (temp.length != 0) {
      ...
      <div>
        {
          countryArr.map((value: any, index: number) => {
            return <CountryCard label={value.countryName} flag={value.countryFlag} count={value.matchCount} competitions_name={value.competitions_name} competitions={value.competitions} key={index + "Cricket_PlayCard"} />
          })
        }
      </div>
    </div>
