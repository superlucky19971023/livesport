...
const BaseballPage: React.FC = () => {
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      try {
        const apiResponse = await fetchData(`baseball/matches/${now.date}/${now.month}/${now.year}`);
        const temp: any[] = [];
        apiResponse.events.forEach((element: any) => {
          if (element.awayTeam.country.name == element.homeTeam.country.name && element.awayTeam.sport.name == "Baseball" && element.homeTeam.sport.name == "Baseball") {
            temp.push(element);
          }
        });
        if (temp.length != 0) {
      ...
      <div>
        {
          countryArr.map((value: any, index: number) => {
            return <CountryCard key={index + "Baseball_PlayCard"} label={value.countryName} flag={value.countryFlag} count={value.matchCount} competitions_name={value.competitions_name} competitions={value.competitions} />
          })
        }
      </div>
    </div>
